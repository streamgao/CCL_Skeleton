...
function drawBarGraph(data) {
	...
	var xScale = d3.time.scale()
		.domain( [  d3.min(data,function(d){ return new Date(d3.keys(d)[0] ); } ), 
					d3.max(data,function(d){ return new Date(d3.keys(d)[0] ); } )
	    ])
		.range( [0, width] );

	var yScale = d3.scale.linear()
			.domain( [ d3.min(data,function(d){ return d3.values(d)[0].totalill; } ),
	...
	svg.append('g')
			...
			.text('Cases of people falling ill from foodbourne illness.');
	...
	svg.selectAll('.bar')
			...
			.append('rect')
			.attr('class','bar')
			.attr('x', function(d) { return xScale( new Date(d3.keys(d)[0] ) ); 
								   } )
			.attr('y', function(d){ return yScale( new Date(d3.values(d)[0].totalill )); } )
			.attr('width', '4')
