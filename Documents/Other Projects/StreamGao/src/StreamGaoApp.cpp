...
void StreamGaoApp::setup()
{
	...
	auto clover = geom::Teapot().subdivisions( 30 );
...
void StreamGaoApp::draw()
{
	...
	gl::ScopedGlslProg shader( gl::context()->getStockShader( gl::ShaderDef().texture().color() ) );
	...
	shader->uniform( "Kd", vec3( 0.5, 0.4, 0.4 ) );
	shader->uniform( "Ks", vec3( 0.6, 0.6, 0.6 ) );
